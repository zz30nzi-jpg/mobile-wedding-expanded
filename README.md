...
사진은 공개 청첩장에 표시하기 위해 Supabase Storage의 공개 버킷에 저장됩니다. 20MB 이하 JPG, PNG, WEBP, GIF, HEIC, HEIF 파일만 업로드할 수 있습니다. 주민등록증처럼 공개되면 안 되는 이미지는 업로드하지 마세요.

## 하객 결혼식 사진

청첩장의 `Wedding Snap` 섹션에서 하객이 휴대폰 갤러리 사진을 여러 장 선택해 업로드할 수 있습니다.

- 사진 원본은 `guest-photos` 비공개 Storage 버킷에 저장됩니다.
- 하객은 별도 가입 없이 이 휴대폰 브라우저에서 자신이 보낸 사진만 확인하고 삭제할 수 있습니다.
- 하객은 다른 하객이 보낸 사진을 볼 수 없습니다.
- 신랑과 신부는 `admin.html`의 `하객 사진` 메뉴에서 전체 사진을 확인하고 삭제할 수 있습니다.
- 관리자 화면의 사진 링크는 30일 동안 유효합니다. 사진 원본은 직접 삭제하기 전까지 비공개 버킷에 유지됩니다.
- `청첩장 편집`에서 미리보기 표시를 끄면 서울 시간 기준 `2026-10-04` 당일에만 화면과 서버 업로드가 열립니다.
- 예식 이후에는 업로드 버튼이 닫히고, 기존 사진을 확인하고 삭제하는 `내가 보낸 사진` 버튼만 유지됩니다.

이전 버전의 `supabase-setup.sql`을 실행했다면 SQL Editor에서 최신 파일을 다시 실행해 주세요. 사진 업로드 정책과 방명록 숨김 관리 기능이 반영됩니다.

하객 사진은 별도 버킷 제한을 두지 않았지만 Supabase 프로젝트의 전역 제한은 적용됩니다. 무료 플랜은 파일당 최대 50MB입니다. 현재 구현은 일반 업로드 방식을 사용하므로 6MB를 넘는 파일은 모바일 네트워크 상태에 따라 업로드가 불안정할 수 있습니다.

하객별 사진 구분에는 Supabase 익명 로그인을 사용합니다. Supabase Dashboard의 Authentication > Providers > Anonymous Sign-Ins를 활성화해야 합니다. 브라우저 데이터 삭제, 시크릿 모드 종료, 휴대폰 변경 후에는 기존 익명 로그인 정보를 복구할 수 없으므로 해당 하객이 기존 사진을 직접 삭제할 수 없습니다. 관리자 화면에서는 계속 확인할 수 있습니다.
...
- `admin.html` 일반관리자: 청첩장 편집, 디자인 적용, 참석 현황, 하객 사진
...
하객 사진은 익명 로그인 세션을 만든 뒤 `guestPhotos.uploadSlug/익명사용자ID/파일명` 경로로 저장합니다. 운영 중인 프로젝트에서 사진 업로드 권한 오류가 발생하면 SQL Editor에서 `supabase-guest-photo-policy-fix.sql`을 실행하세요. Storage RLS는 경로 문자열 대신 Storage가 자동으로 기록하는 `owner_id`와 현재 로그인 사용자의 `auth.uid()`를 비교합니다. `guestPhotos.eventDate`와 `guestPhotos.previewVisible`은 화면에서 업로드 버튼 노출 시점을 제어합니다.
...
10. 청첩장 페이지를 새로고침한 뒤 웨딩스냅 사진을 다시 업로드합니다.
